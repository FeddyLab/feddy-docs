...
Call `identify` with your own user id to turn the anonymous contact into a known one. The inbox then shows the user's name or email, and their conversations from every device merge into one history. How to call it, and what happens on merge, is in [Identify users](/docs/guides/identify-users/).
...
A contact's email comes from `identify` or from the user typing it into the widget. It is used for one thing: emailing them your replies, and only when your project has a verified sending domain. See [Team](/docs/concepts/team/).
